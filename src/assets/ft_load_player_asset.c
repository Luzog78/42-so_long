...
/*   Updated: 2023/11/29 15:13:18 by ysabik           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_load_player_asset(t_data *data, int idx, char *path, int amount)
{
	...
	while (i < amount)
	{
		itoa = ft_itoa(i, 2);
		if (!itoa)
			return (ft_error(data, "Error: Can't allocate memory for itoa\n"));
...
static int	ft_load(t_data *data, int idx, int i, char *path)
{
	...
	if (!complete_path)
		return (ft_error(data, "Error: Can't allocate memory for path\n"));
	...
	if (!data->player_assets[idx].frames[i].img)
		return (ft_error(data, "Error: Can't load player asset\n"));
	...
	if (!data->player_assets[idx].frames[i].addr)
		return (ft_error(data, "Error: Can't get player asset address\n"));
	return (0);
}
